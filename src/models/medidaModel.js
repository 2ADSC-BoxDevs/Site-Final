...
function buscarUltimasMedidas(idEmpresa) {
    instrucaoSql = `SELECT TOP 5 maquina.*, historico_maquina.* FROM maquina JOIN historico_maquina ON historico_maquina.fk_maquina=maquina.id_maquina WHERE id_maquina = ${idEmpresa} ORDER BY historico_maquina.id_historico DESC;`;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}

function buscarMedidasEmTempoReal(idEmpresa) {
    instrucaoSql = `SELECT TOP 1 maquina.*, historico_maquina.* FROM maquina JOIN historico_maquina ON historico_maquina.fk_maquina=maquina.id_maquina WHERE id_maquina = ${idEmpresa} ORDER BY historico_maquina.id_historico DESC;`;
